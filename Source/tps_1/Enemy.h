...
UCLASS()
class TPS_1_API AEnemy : public ACharacter , public IBulletHitInterface
{
	GENERATED_BODY()
...
protected:
	...
	// implementation fully in blueprints
...
private:
	...
	// for headshot damage the head bone is required , as different characters have diff bone name
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Combat, meta = (AllowPrivateAccess = "true"))
		FString HeadBone;
	...
	// these are created cause if we spam the fire button the animation will start playing 
	//repeatedly so to prevent that we use timer 
...
public:
	...
	// native events needs to have a _Implementation name in the end 
	virtual void BulletHit_Implementation(FHitResult HitResult);
	//, AActor* Shooter, AController* ShooterController) override;

	//Take Damage is an in built function in unreal engine for a actor , inherited from aactor
